...
export class WorldpayAdapter implements PaymentProvider {
  ...
  async createOrder(req: NormalizedPaymentOrderRequest): Promise<NormalizedPaymentOrderResponse> {
    if (process.env.NODE_ENV === 'production' && (!this.serviceKey)) {
    ...
    if (!req.providerSignature || !key) {
      return {
        ...
        status: 'FAILED',
        failureReason: 'Missing Worldpay payment signature or secret key',
    ...
    return {
      ...
      failureReason: isValid ? undefined : 'Worldpay signature verification failed',
      rawResponse: {
