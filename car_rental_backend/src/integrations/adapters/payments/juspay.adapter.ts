...
export class JuspayAdapter implements PaymentProvider {
  ...
  async createOrder(req: NormalizedPaymentOrderRequest): Promise<NormalizedPaymentOrderResponse> {
    if (process.env.NODE_ENV === 'production' && (!this.merchantId || !this.apiKey)) {
    ...
    const key = this.webhookKey || this.apiKey;
    if (!req.providerSignature || !key) {
      return {
        ...
        status: 'FAILED',
        failureReason: 'Missing Juspay payment signature or secret key',
    ...
    return {
      ...
      failureReason: isValid ? undefined : 'Juspay signature verification failed',
      rawResponse: {
