...
@Injectable()
export class NetellerAdapter implements PaymentProvider {
  private readonly logger = new Logger(NetellerAdapter.name);
  private merchantId: string;
  private secretKey: string;

  constructor(private readonly configService: ConfigService) {
    this.merchantId = this.configService.get<string>('NETELLER_MERCHANT_ID') || '';
    this.secretKey = this.configService.get<string>('NETELLER_SECRET_KEY') || '';
  }
  ...
  getDisplayName(): string {
    return 'Neteller Paysafe Digital Payments';
  }
  ...
  async testConnection(credentials?: Record<string, any>): Promise<TestConnectionResult> {
    ...
    if (!mId || !sec) {
      return {
        ...
        message: 'Neteller credentials not configured',
      };
    }
    return {
      success: true,
      latencyMs: Date.now() - start,
      message: 'Neteller connection verified',
    };
  }
  ...
  async createOrder(req: NormalizedPaymentOrderRequest): Promise<NormalizedPaymentOrderResponse> {
    ...
    const orderId = `net_ord_${req.bookingId}_${Date.now()}`;
    const amountFloat = (req.amountPaise / 100).toFixed(2);
    this.logger.log(`[NETELLER] Created payment order ${orderId} for ${req.currency} ${amountFloat}`);
    ...
    return {
      ...
      rawResponse: {
        ...
        amount: amountFloat,
        currency: req.currency || 'EUR',
        redirect_url: `https://pay.neteller.com/checkout?order=${orderId}`,
      },
    };
  }
    ...
    if (!req.providerSignature || !key) {
      return {
        ...
        status: 'FAILED',
        failureReason: 'Missing Neteller payment signature or secret key',
    ...
    return {
      ...
      failureReason: isValid ? undefined : 'Neteller signature verification failed',
      rawResponse: {
  ...
  async refund(req: NormalizedRefundRequest): Promise<NormalizedRefundResponse> {
    ...
    const refundId = `net_ref_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`;
    this.logger.log(`[NETELLER] Refund ${refundId} completed for ${req.providerPaymentId}`);
    return {
  ...
  normalizeWebhook(rawPayload: any): NormalizedPaymentWebhookEvent {
    const isSuccess = rawPayload?.status === 'COMPLETED' || rawPayload?.event === 'payment.completed';
    return {
      eventId: rawPayload?.id || `net_evt_${Date.now()}`,
