...
export class TwoCheckoutAdapter implements PaymentProvider {
  ...
  async createOrder(req: NormalizedPaymentOrderRequest): Promise<NormalizedPaymentOrderResponse> {
    if (process.env.NODE_ENV === 'production' && (!this.merchantCode || !this.secretKey)) {
    ...
    if (!req.providerSignature || !key) {
      return {
        ...
        status: 'FAILED',
        failureReason: 'Missing 2Checkout payment signature or secret key',
    ...
    return {
      ...
      failureReason: isValid ? undefined : '2Checkout signature verification failed',
      rawResponse: {
