...
export class BraintreeAdapter implements PaymentProvider {
  ...
  async createOrder(req: NormalizedPaymentOrderRequest): Promise<NormalizedPaymentOrderResponse> {
    if (process.env.NODE_ENV === 'production' && (!this.merchantId || !this.privateKey)) {
    ...
    if (!req.providerSignature || !key) {
      return {
        ...
        status: 'FAILED',
        failureReason: 'Missing Braintree payment signature or secret key',
    ...
    return {
      ...
      failureReason: isValid ? undefined : 'Braintree signature verification failed',
      rawResponse: {
